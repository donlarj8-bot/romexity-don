...
import { client } from '@/sanity/lib/client'; // Import your Sanity bridge
...
// Updated Type to match Sanity's data structure
type Shop = {
  _id: string;      // Sanity uses _id
  name: string;
  loc: string;
  img: string;      // This will be the URL from Sanity
  tags: string;
  description: string;
};
...
export default function Home() {
  ...
  // 1. Create a state to hold your live shops
  const [shops, setShops] = useState<Shop[]>([]);
  const [loading, setLoading] = useState(true);

  // 2. Fetch data from Sanity when the page loads
  useEffect(() => {
  ...
  // 3. Filter logic now uses the 'shops' from Sanity
